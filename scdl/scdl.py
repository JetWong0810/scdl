...
def download_track(client: SoundCloud, track: BasicTrack, playlist_info=None, exit_on_fail=True, **kwargs):
    ...
    try:
        ...
        #保存到数据库
        #save_track_info(track, filename)
        update_mp3_info(track, filename)
...
def save_track_info(track: BasicTrack, filename: str):
    track.title = pymysql.converters.escape_string(str(track.title))
    info_insert_item = (track.id, track.user.id, filename, track.title, track.permalink_url, track.duration, track.genre, track.kind, track.license, track.likes_count, track.tag_list)
    user_insert_item = (track.user.id, track.user.username, track.user.followers_count, track.user.city, track.user.country_code, track.user.permalink_url)
    ...
    try:
        ...
        logger.info(f"插入mp3记录成功.\n")
    except Exception as err:
        logger.error(err)
        logger.error(f"插入mp3数据失败,失败id:{track.id}")
    ...
    try:
        ...
        conn.commit()
        logger.info(f"插入用户记录成功.\n")
    except Exception as err:
        logger.error(err)
        logger.error(f"插入用户数据失败，用户可能已存在，用户id:{track.user.id}")
    
    cur.close()
    conn.close()

def update_mp3_info(track: BasicTrack, filename: str):
    try:
        cur.execute(
            "UPDATE mp3_info SET is_download = 1, filename = '%s' where audio_id = '%s'"
            % (filename, track.id))

        conn.commit()

        logger.info(f"更新mp3下载状态成功:{track.id}")
    except Exception as err:
        logger.error(err)
        logger.error(f"更新mp3下载状态失败:{track.id}")
